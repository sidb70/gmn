    ...
    **random_cnn_kwargs
    ...
    if train_size is None:
        ...
        train_sampler = SubsetRandomSampler(
            torch.randperm(len(trainset))[:train_size])
    ...
    if train_size is None:
        ...
        test_sampler = SubsetRandomSampler(
            torch.randperm(len(testset))[:test_size])
    ...
    if replace_if_existing:
        ...
        if os.path.exists(os.path.join(directory, 'features.pt')) and os.path.exists(os.path.join(directory, 'accuracies.pt')):
            # load
            features = torch.load(os.path.join(
                directory, 'features.pt')) + features
            accuracies = torch.load(os.path.join(
                directory, 'accuracies.pt')) + accuracies
