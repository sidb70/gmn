...
class LayerFactory:
    ...
    def create_input_layer(self, module: nn.Module, **kwargs) -> NetworkLayer:
        ...
        for i in range(num_input_nodes):
            node_features = NodeFeatures(
                layer_num=0, rel_index=i, node_type=NodeType.INPUT
            )
    ...
    def create_linear_layer(
        self, layer: nn.Linear, layer_num: int, start_node_id: int, **kwargs
    ) -> NetworkLayer:
        """
        ...
        for i, weights in enumerate(layer.weight.data):
            ...
            edge = Edge(
                (bias_node, node),
                EdgeFeatures(
                    weight=layer.bias.data[i],
                    layer_num=layer_num,
                    edge_type=EdgeType.LIN_BIAS,
                ),
            )
            linear_layer.add_edge(edge)
    ...
    def create_norm_layer(
        self, module: nn.Module, layer_num: int, start_node_id: int, **kwargs
    ) -> NetworkLayer:
        """
    ...
    def create_conv_layer(
        self, module: nn.Conv2d, layer_num: int, start_node_id: int, **kwargs
    ) -> NetworkLayer:
        """
        ...
        for out_channel in range(module.out_channels):
            ...
            out_channel_node = Node(
                node_id,
                NodeFeatures(
                    layer_num=layer_num, rel_index=-1, node_type=NodeType.CONV
                ),
            )
            ...
            for idx, in_node_id in enumerate(prev_layer.nodes):
                ...
                for weight in weights:
                    ...
                    j += 1
                bias_edge = Edge(
                    (in_node_id, out_channel_node),
                    EdgeFeatures(
                        weight=module.bias[out_channel],
                        layer_num=layer_num,
    ...
    def create_non_parametric_layer(
        self, module: nn.Module, layer_num: int, start_node_id: int, **kwargs
    ) -> NetworkLayer:
        """
        ...
        node = Node(
            start_node_id,
            NodeFeatures(
                layer_num=layer_num, rel_index=0, node_type=NodeType.NON_PARAMETRIC
            ),
        )
    ...
    def module_to_graph(
        ...
        if module_type == LayerType.LINEAR:
            ...
            return self.create_non_parametric_layer(
                module, layer_num, start_node_id, **kwargs
            )
        else:
            raise ValueError(f"Layer type {type(module)} not yet supported")
