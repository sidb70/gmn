...
class EdgeFeatures(BaseModel):
    ...
    edge_type: EdgeType
    pos_encoding_x: int = (
        0  # x positional encoding of this parameter within the conv layer
    )
    pos_encoding_y: int = (
        0  # y positional encoding of this parameter within the conv layer
    )
...
class ParameterGraph(nx.MultiDiGraph):
    ...
    def serialize(self) -> dict:
        ...
        """
        nodes = [
            {"id": node_id, "features": node_obj.features.serialize()}
            for node_id, node_obj in self.nodes(data="node_obj")
        ]
        edges = [
            {
                "source": source,
                "target": target,
                "features": edge_obj.features.serialize(),
            }
            for source, target, edge_obj in self.edges(data="edge_obj")
        ]
        ser = {"nodes": nodes, "links": edges}
        return ser
    ...
    def get_bipartite_adjacency_matrix(self) -> torch.Tensor:
        ...
        adj = nx.bipartite.biadjacency_matrix(
            self, row_order=set_a, column_order=set_b
        ).toarray()
...
class Node:
    ...
    def serialize(self) -> dict:
        ...
        """
        ser = {"node_id": self.node_id, "features": self.features.serialize()}
        return ser
...
if __name__ == "__main__":
    testnode = Node(
        node_id=0,
        features=NodeFeatures(layer_num=0, rel_index=0, node_type=NodeType.INPUT),
    )
    testedge = Edge(
        connection_nodes=(testnode, testnode),
        features=EdgeFeatures(
            weight=0.5,
            layer_num=0,
