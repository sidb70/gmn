...
def draw_nx_graph(graph: ParameterGraph, title: str):
    ...
    for u, v, data in graph.edges(data=True):
        ...
        nx.draw_networkx_edges(
            graph, pos, edgelist=[(u, v)], connectionstyle=f"arc3,rad={rad}"
        )
        i += 1
    plt.title(title)
    plt.show()
...
def draw_3d_graph(
    graph: nx.Graph, title: str, save_path: str = None, display_inline=False
) -> None:
    ...
    node_to_index = {
        node_id: i for i, (node_id, _) in enumerate(graph.nodes(data=True))
    }

    # Create a color map for edge types
    unique_edge_types = set(
        str(data["edge_obj"].features.edge_type)
    ...
    edge_type_to_color = {
        edge_type: color_map[i % len(color_map)]
