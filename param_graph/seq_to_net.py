...
def seq_to_nx(seq: nn.Sequential) -> ParameterGraph:
    ...
    for module in seq:
        if type(module) in [nn.Flatten, nn.AdaptiveAvgPool1d, nn.AdaptiveAvgPool2d, nn.Identity]:
            continue
        layer = layer_factory.module_to_graph(
            module, layer_num=layer_num, start_node_id=starting_node_id, prev_layer=prev_layer)
        ...
        for edge in layer.get_edges():
            global_graph.add_edge(edge.node1.node_id,
                                  edge.node2.node_id, edge_obj=edge)
